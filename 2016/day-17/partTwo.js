...
const findLongestPath = (start, end, passcode) => {
  const queue = [];
  ...
  while (queue.length > 0) {
    ...
    if (currentLocation.x === end.x && currentLocation.y === end.y) {
      if (currentLocation.path.length > longestPath.length) {
        longestPath = currentLocation.path;
      }
      continue;
    }
...
const solvePuzzle = (passcode) => {
  return findLongestPath({ x: 0, y: 0 }, { x: 3, y: 3 }, passcode);
};
