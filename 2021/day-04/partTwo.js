...
const main = async () => {
  try {
    ...
    // Set to store the index of the winning boards
    const winningBoards = new Set();

    // Current winningNum and winningBoard
    let winningNum;
    let winningBoard;
    ...
    for (let i = 0; i < numbers.length; i++) {
      ...
      for (let j = 0; j < boards.length; j++) {
        ...
        if (!winningBoards.has(j) && isWinner(markedBoard)) {
          winningNum = num;
    ...
    const boardSum = sumBoard(winningBoard);

    console.log(boardSum * winningNum);
  } catch (err) {
    console.error(err);
  }
};
